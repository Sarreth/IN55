...
//    this->path = new Path("resources/path/path.png");
    ...
    normals = new QVector3D[(this->numberVerticesX-1) * (this->numberVerticesZ-1) * 2];
    ...
    for(int z = 0 ; z < this->numberVerticesZ - 1 ; ++z){
        for(int x = 0 ; x < this->numberVerticesX - 1 ; ++x){
            ...
            //first triangle
            //initialize u vector
            u = QVector3D(verticesTab[indiceVer] - verticesTab[indiceHoriz],
                  verticesTab[indiceVer + 1] - verticesTab[indiceHoriz + 1],
                  verticesTab[indiceVer + 2] - verticesTab[indiceHoriz + 2]);
            u.normalize();
            //initialize v vector
            v = QVector3D(verticesTab[indiceHoriz + 3] - verticesTab[indiceHoriz],
            ...
            normals[j] = QVector3D::crossProduct(u,v);

            //second triangle
            //initialize u vector
            u = QVector3D(verticesTab[indiceHoriz + 3] - verticesTab[indiceVer + 3],
                  verticesTab[indiceHoriz + 4] - verticesTab[indiceVer + 4],
                  verticesTab[indiceHoriz + 5] - verticesTab[indiceVer + 5]);
            u.normalize();

            //initialize v vector
            v = QVector3D(verticesTab[indiceVer] - verticesTab[indiceVer + 3],
                  verticesTab[indiceVer + 1] - verticesTab[indiceVer + 4],
                  verticesTab[indiceVer + 2] - verticesTab[indiceVer + 5]);
            v.normalize();

            //set the normal in the normals array
            normals[j+1] = QVector3D::crossProduct(u,v);
            i += 6;
            j+=2;
        }
    }
...
void Ground::setVectorToFloat(QVector3D normals, float * normalsF, int i){
...
float Ground::getY(float px, float pz){
    ...
    if(px < MAP_SIZE / 2 && px > -MAP_SIZE / 2 && pz < MAP_SIZE / 2 && pz > -MAP_SIZE / 2){
        ...
        p = QVector3D(this->geometry->getVertices()[(indx + indz * this->numberVerticesX) * 3],
...
QVector3D Ground::randomMapPos(double height, bool isRock)
{
    double x = 0;
    double z = 0;
    int samples = 20;
...
//    do
//    {
//        x = (rand() / static_cast<double>(RAND_MAX)) * (MAP_MAX - MAP_MIN) + MAP_MIN;
//        z = (rand() / static_cast<double>(RAND_MAX)) * (MAP_MAX - MAP_MIN) + MAP_MIN;
//    } while ((!this->path->validate(x,z) || (!isRock && this->containInBoundingBox(x,z))) && samples-- > 0);

    if (isRock)
        rockPos.append(new QVector3D(x,0,z));
...
bool Ground::containInBoundingBox(double x, double z)
{
    ...
    foreach (QVector3D* rock, this->rockPos) {
...
QList<QVector3D *> Ground::getRockPos()
{
    return this->rockPos;
}
