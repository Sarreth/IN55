...
PostComposer::PostComposer(FrameBuffer *inputFBO)
{
    ...
    gammaRGB = QVector3D(1.0, 1.0, 1.0);

    flareThreshold = QVector3D(0.5,0.5,0.5);
    flareSamples = 12;
    flareDispersal = 0.10;
    flareHaloWidth = 0.22;
    flareChromaDispertion = QVector3D(1.02,1.0,0.98);
    flareIntensity = 1.0;
    flareDirt = new Texture("pics/post/lensdirt_highc.jpg", 2);
    flareDirt->init();
}
...
void PostComposer::passGodrays()
{
    ...
    // Pass 1.b : blur   passBlurLinear(true);
...
{
    gammaRGB = QVector3D(value, value, value);
}

void PostComposer::setLightPosScreen(const QVector3D &pos)
{
    lightPosScreen = pos ;
