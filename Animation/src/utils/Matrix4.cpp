...
Matrix4& Matrix4::lookAt(const QVector3D &eye, const QVector3D &target, const QVector3D &up)
{
    QVector3D x, y, z;
    ...
    x = QVector3D::crossProduct(z, up);
    x.normalize();
    y = QVector3D::crossProduct(x, z);
    y.normalize();
...
Matrix4& Matrix4::lookAt2(const QVector3D &eye, const QVector3D &target, const QVector3D &up)
{
    QVector3D x, y, z;
    ...
    data[0][3] = -QVector3D::dotProduct(x,eye);
    data[1][3] = -QVector3D::dotProduct(y,eye);
    data[2][3] = QVector3D::dotProduct(z,eye);
...
Matrix4& Matrix4::setRotationMatrix(float angle, const QVector3D &xyz)
{
...
Matrix4& Matrix4::setRotationMatrix(const QQuaternion &q)
{
    float x = q.x(),
          y = q.y(),
          z = q.z(),
...
QVector3D& Matrix4::rotateAxis(QVector3D &axis)
{
    ...
    */
    axis.setX(vx * data[0][0] + vy * data[0][1] + vz * data[0][2]);
    axis.setY(vx * data[1][0] + vy * data[1][1] + vz * data[1][2]);
    axis.setZ(vx * data[2][0] + vy * data[2][1] + vz * data[2][2]);
...
Matrix4& Matrix4::setPosition(const QVector3D &xyz)
{
    data[0][3] = xyz.x();
    data[1][3] = xyz.y();
    data[2][3] = xyz.z();
...
Matrix4& Matrix4::translate(const QVector3D &xyz)
{
...
Matrix4& Matrix4::scale(const QVector3D &xyz)
{
...
Matrix4& Matrix4::rotate(float angle, const QVector3D &xyz)
{
...
QVector3D Matrix4::operator *(const QVector3D vec3) const
{
    return QVector3D(data[0][0] * vec3.x() + data[0][1] * vec3.y() + data[0][2] * vec3.z() + data[0][3],
