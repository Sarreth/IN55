...
void OpenGLWidget::initializeObjects()
{
    renderer = new Renderer();
    renderer->loadShaders();

    qDebug() << "+ Creation du Renderer : OK";

    scene = new Scene();

    qDebug() << "+ Creation de la scene : OK";

    scene->init();

    qDebug() << "+ Init de la scene : OK";


    renderer->init(scene, 1024, 768);

    qDebug() << "+ Init du renderer : ok";
...
void OpenGLWidget::keyPressEvent(QKeyEvent *event)
{
    switch(event->key())
    {
        ...
        case Qt::Key_Z:
            ...
            break;
        case Qt::Key_Space:
            scene->getCurrentCamera()->setSpaceKey(true);
            break;
        case Qt::Key_Control:
            scene->getCurrentCamera()->setControlKey(true);
            break;
        default:
            break;
    }
...
void OpenGLWidget::keyReleaseEvent(QKeyEvent *event)
{
    switch(event->key())
    {
        ...
        case Qt::Key_Z:
            ...
            break;
        case Qt::Key_Space:
            scene->getCurrentCamera()->setSpaceKey(false);
            event->accept();
            break;
        case Qt::Key_Control:
            scene->getCurrentCamera()->setControlKey(false);
            event->accept();
            break;
        default:
            break;
